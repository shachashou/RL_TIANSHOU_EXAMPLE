...
from ENV.frozenlake import FrozenLakeWrapper
...
TRAINER_STEP_PER_EPOCH = 1000  # 一轮做题数
TRAINER_STEP_PER_COLLECT = 100  # 多少题学习一次
TRAINER_UPDATE_PER_STEP = 1  # 反思强度
TRAINER_EPISODE_PER_TEST = 100
TRAINER_BATCH_SIZE = 64
TRAINER_EPS_TRAIN_START = 1.0  # 训练开始时的探索率 (完全随机)
TRAINER_EPS_TRAIN_END = 0.1  # 训练结束时的最低探索率 (接近贪心)
TRAINER_EPS_DECAY_STEPS = 5000

POLICY_TEST_EPS = 0.001
POLICY_LR = 1e-3  # 学习率
POLICY_DISCOUNT_FACTOR = 0.9  # 远见程度， 越接近1越有远见
POLICY_ESTIMATION_STEP = 3  # 往后看多步答案 ，简单任务就是3,迷宫的话，可能是10或者15
POLICY_TARGET_UPDATE_FREQ = 100

env = FrozenLakeWrapper()
train_envs = ts.env.DummyVectorEnv([lambda: FrozenLakeWrapper() for _ in range(ENV_TRAIN_NUM)])
test_envs = ts.env.DummyVectorEnv([lambda: FrozenLakeWrapper() for _ in range(ENV_TEST_NUM)])
