...
TRAINER_STEP_PER_EPOCH = 10000  # 一轮做题数
TRAINER_STEP_PER_COLLECT = 10  # 多少题学习一次
TRAINER_UPDATE_PER_STEP = 0.1  # 反思强度
TRAINER_EPISODE_PER_TEST = 100
TRAINER_BATCH_SIZE = 64

POLICY_TRAIN_EPS = 0.1  # 探索率
...
env = gym.make(ENV_ID)
train_envs = ts.env.DummyVectorEnv([lambda: gym.make(ENV_ID) for _ in range(ENV_TRAIN_NUM)])
test_envs = ts.env.DummyVectorEnv([lambda: gym.make(ENV_ID) for _ in range(ENV_TEST_NUM)])
...
result = ts.trainer.OffpolicyTrainer(
    ...
    train_fn=lambda epoch, env_step: policy.set_eps(POLICY_TRAIN_EPS),
