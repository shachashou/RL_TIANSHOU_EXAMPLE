...
POLICY_TARGET_UPDATE_FREQ = 16000
...
env = AtariWrapper(ENV_ID, resize_height=84, resize_width=84)
train_envs = ts.env.DummyVectorEnv([lambda: AtariWrapper(ENV_ID, resize_height=84, resize_width=84) for _ in range(ENV_TRAIN_NUM)])
test_envs = ts.env.DummyVectorEnv([lambda: AtariWrapper(ENV_ID, resize_height=84, resize_width=84) for _ in range(ENV_TEST_NUM)])
